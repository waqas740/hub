...
const init = {
    ...
    createApp(args) {
        ...
        const destinationPath = args._[1] || "hub-app"
        const program = args
        // App name
        const appName = this.createAppName(path.resolve(destinationPath)) || 'hub-app';
        ...
        const hubrc = helpers.template.loadTemplate("templates/hubrc.ejs", { appName });
        helpers.asset.write(helpers.path.join(appName, '.hubrc'), hubrc)
        ...
        helpers.view.log(` $ cd ${appName} && npm install && npm run sequelize init && npm start`);
        return;
    ...
    createPackageJsonFile(name) {
        this.pkg = {
            ...
            private: true,
            scripts: {
                start: "open-cli http://localhost:3000 && node ./bin/www",
                sequelize: "sequelize",
    ...
    createWelcomeView(dir) {
        ...
        helpers.asset.copy("templates/views/pages/index.ejs", helpers.path.join(dir, 'views/pages/index.ejs'))
        helpers.asset.copy("templates/views/pages/error.ejs", helpers.path.join(dir, 'views/pages/error.ejs'))
    },
